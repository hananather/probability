...
const FormulasSection = ({ expandedFormula, setExpandedFormula }) => {
  const contentRef = useMathJax([expandedFormula]);
...
const MistakesSection = ({ expandedMistake, setExpandedMistake }) => {
  const contentRef = useMathJax([expandedMistake]);
      ...
      <div ref={contentRef}>
        <WorkedExample title="Avoid These Common Pitfalls">
          ...
          <InsightBox variant="warning">
            Most Common Error: Not recognizing whether replacement is allowed. 
...
const SpeedPracticeSection = ({ 
  ...
  const contentRef = useMathJax([currentProblem, showResults]);
