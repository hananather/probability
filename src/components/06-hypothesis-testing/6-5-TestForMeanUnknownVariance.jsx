...
export function TestForMeanUnknownVariance() {
  ...
  // Memoized distribution data - moved from renderDistributionStep
  ...
  const goToStep = useCallback((stepIndex) => {
    ...
    // Reset calculation step state when leaving or entering step 3
      ...
      >
        ...
        <div className="relative bg-gray-900/90 backdrop-blur-xl rounded-2xl p-8 m-[2px]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6">
            ...
            <InfoCard 
              title="Sample Mean" 
              value={<span dangerouslySetInnerHTML={{ __html: `\\(\\bar{x} = ${sampleMean.toFixed(1)}\\)` }} />}
              subtitle={<span>vs claimed <span dangerouslySetInnerHTML={{ __html: `\\(\\mu_0 = 16.6\\)` }} /></span>}
              color="primary"
              icon={TrendingUp}
              delay={0.6}
            />
  ...
  // Effect for comparison step visualization
  useEffect(() => {
    if (!comparisonRef.current || currentStep !== 1) return;
      ...
      const boxes = [
        { 
          ...
          data: [
            { label: "Test Statistic", value: "Z = (\\bar{X} - \\mu_0)/(\\sigma/\\sqrt{n})", math: true },
        ...
        { 
          ...
          data: [
            { label: "Test Statistic", value: "T = (\\bar{X} - \\mu_0)/(S/\\sqrt{n})", math: true },
  ...
  // Effect for distribution step visualization
  useEffect(() => {
  ...
  const renderCalculationStep = () => {
    const testSteps = [
      ...
      },
      {
        ...
        result: "df = 15",
        explanation: "We lose one degree of freedom because we estimated \u03bc with \\bar{x}."
      },
      {
        title: "Determine p-value",
        formula: "\\(P(T_{15} > 2.968)\\)",
        result: "0.0025 < p-value < 0.005",
        explanation: "The probability of seeing data this extreme if \\(H_0\\) is true."
      },
      {
        title: "Make Decision",
