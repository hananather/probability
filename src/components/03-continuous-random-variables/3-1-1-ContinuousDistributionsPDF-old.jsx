...
import { useD3Drag } from '../../hooks/useD3Drag';
import { 
  ...
  useEffect(() => {
    if (stage.distribution && stage.params) {
      ...
      if (stage.distribution === 'uniform') {
        ...
        // For step 5 specifically, use μ ± σ to show the 68% rule
        if (currentStage === 4) {
  ...
  useEffect(() => {
    ...
    switch (stage.distribution) {
      case 'uniform':
        ...
        break;
      case 'normal':
        // Calculate max based on current σ to prevent cutoff
        // For normal distribution, max density = 1/(σ*sqrt(2π))
        const σ = params[1] || 1;
    ...
    // Store refs for draggable elements
    const markerARefs = useRef({ group: null, line: null, circle: null, text: null });
    const markerBRefs = useRef({ group: null, line: null, circle: null, text: null });
