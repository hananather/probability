...
import DistributionSimulationWrapper from '../components/DistributionSimulationWrapper.jsx';
import CLTSimulationWrapper from '../components/CLTSimulationWrapper.jsx';
...
import BayesianInference from '../components/BayesianInference';
...
**Theorem (Central Limit Theorem).** Let X̄ be the sample mean of an i.i.d. sample of size n drawn from an unknown population with finite mean μ and variance σ². Then as n → ∞, the standardized variable
...
- The Xᵢ are independent and identically distributed (i.i.d.).
- E[Xᵢ] = μ is finite.
- Var(Xᵢ) = σ² is finite.

**Remarks:**
- It works even if the original Xᵢ are not normal.
- Convergence speed depends on skewness/kurtosis; n ≥ 30 is a common rule of thumb.
